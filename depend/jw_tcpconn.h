...
public:
	...
	void OnRead(std::function<void(TcpConn*, char*, int32_t)> read_callback) { read_callback_ = read_callback; }
private:
	int conn_state_;
	JwSocket socket_;

	std::vector<Buffer*> recv_data_;
	...
	std::function<void(TcpConn*, char*, int32_t)> read_callback_;
};
